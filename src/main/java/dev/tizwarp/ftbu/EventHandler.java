...
import dev.tizwarp.ftbu.compat.jei.CompatJEI;
import dev.tizwarp.ftbu.event.PlayerLockEvent;
...
import dev.tizwarp.ftbu.proxy.ProxyClient;
...
import dev.tizwarp.ftbu.util.LootUtils;
...
public class EventHandler {
	...
	public void onKey(InputEvent.KeyInputEvent event) {
		if (ProxyClient.key.isPressed()) {
			FTBU.PROXY.openResearchBook(Minecraft.getMinecraft().player);
	...
	public void onPlayerJoin(PlayerLoggedInEvent evt) {
		if (!evt.player.world.isRemote) {
			...
			if (cap != null) {
				...
				if (cap.isNew()) {
					if (FTBUConfig.giveResearchBook) {
	...
	public void onLootTableLoad(LootTableLoadEvent event) {
		if (event.getName().toString().equals("minecraft:chests/village_blacksmith"))
			LootUtils.addLootPools(event.getLootTableManager(), event.getTable(),
					new ResourceLocation(FTBU.MODID, "inject/blacksmith"));
		if (event.getName().toString().equals("minecraft:chests/desert_pyramid"))
			LootUtils.addLootPools(event.getLootTableManager(), event.getTable(),
					new ResourceLocation(FTBU.MODID, "inject/pyramid"));
		if (event.getName().toString().equals("minecraft:chests/stronghold_library"))
			LootUtils.addLootPools(event.getLootTableManager(), event.getTable(),
					new ResourceLocation(FTBU.MODID, "inject/library"));
	}
	...
	public void onTick(TickEvent.ClientTickEvent event) {
		if (event.phase == TickEvent.Phase.START && FTBU.JEI_LOADED)
