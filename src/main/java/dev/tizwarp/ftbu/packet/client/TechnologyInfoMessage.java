package dev.tizwarp.ftbu.packet.client;
...
public class TechnologyInfoMessage implements IMessage {
	...
	private FTBUConfig.HideJeiItems jeiHide;
	...
	public TechnologyInfoMessage(Map<String, Pair<String, Map<ResourceLocation, String>>> json) {
		this.allowResearchCopy = FTBUConfig.allowResearchCopy;
		this.loadDefaultTechnologies = FTBUConfig.loadDefaultTechnologies;
		this.jeiHide = FTBUConfig.jeiHide;
		this.json = json;
	}
	...
	public void fromBytes(ByteBuf buf) {
		...
		jeiHide = FTBUConfig.HideJeiItems.values()[buf.readByte()];
	...
	public static class TechnologyInfoMessageHandler implements IMessageHandler<TechnologyInfoMessage, IMessage> {
		...
		public IMessage onMessage(TechnologyInfoMessage message, MessageContext ctx) {
			if (FMLClientHandler.instance().getServer() == null) {
				FTBUConfig.allowResearchCopy = message.allowResearchCopy;
				FTBUConfig.loadDefaultTechnologies = message.loadDefaultTechnologies;
				if (FTBUConfig.jeiHide != message.jeiHide)
					FTBUConfig.jeiHide = message.jeiHide;
			...
			FTBU.PROXY.clearToasts(); // Removes unnecessary recipe toasts
