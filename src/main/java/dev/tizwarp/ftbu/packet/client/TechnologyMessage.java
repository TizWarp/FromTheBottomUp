...
import dev.tizwarp.ftbu.api.event.FTBUClientSyncEvent;
		...
		public IMessage handleMessage(EntityPlayer player, TechnologyMessage message) {
			...
			if (cap != null) {
				...
				for (ITechnology toast : message.toasts)
					FTBU.PROXY.displayToastTechnology(toast);

				if(FTBU.JEI_LOADED)
					CompatJEI.refreshHiddenItems(false);
				MinecraftForge.EVENT_BUS.post(new FTBUClientSyncEvent.Post());
			}
