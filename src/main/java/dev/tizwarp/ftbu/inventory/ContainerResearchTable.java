package dev.tizwarp.ftbu.inventory;
...
import dev.tizwarp.ftbu.api.inventory.SlotSpecial;
...
import dev.tizwarp.ftbu.tileentity.TileEntityInventory;
...
public class ContainerResearchTable extends ContainerResearch {
	...
	private int addSlots(TileEntityInventory tileEntity) {
		...
		addSlotToContainer(new SlotCrafting(this, tileEntity, c, 8, 46, 1, OreDictionary.getOres("artisansQuill")));
		feather = c;
		c++;
			...
			if (slotIndex == output) {
				...
				if (itemStack2.getItem() == Content.i_parchmentIdea) {
					if (!mergeItemStack(itemStack2, parchment, parchment + 1, false))
						return ItemStack.EMPTY;
				} else if (ArrayUtils.contains(OreDictionary.getOreIDs(itemStack2), OreDictionary.getOreID("artisansQuill")))
