package dev.tizwarp.ftbu.compat.jei;
...
public class CompatJEI implements IModPlugin {
	...
	public static boolean refreshHiddenItems(boolean refreshCheatItems) {
		...
		if (!cheatItemsEnabled && !stacks.isEmpty() && FTBUConfig.jeiHide == FTBUConfig.HideJeiItems.LOCKED_RECIPES_AND_ITEMS) {
		...
		for (IRecipeWrapper wrapper : wrappers) {
			...
			if (FTBUConfig.jeiHide != FTBUConfig.HideJeiItems.NOTHING && outputs.size() > 0
