...
import dev.tizwarp.ftbu.event.PlayerInspectEvent;
...
public class ItemMagnifyingGlass extends Item {
	...
	public static List<BlockSerializable> getInspected(ItemStack item) {
		...
		NBTTagList blocks = StackUtils.INSTANCE.getItemData(item).getTagList("FTBU", NBT.TAG_COMPOUND);
		...
		if (player.isSneaking()) {
			if (!world.isRemote) {
				...
				if (event.isCanceled()) {
					...
					NBTTagList nbt = tag.getTagList("FTBU", NBT.TAG_COMPOUND);
