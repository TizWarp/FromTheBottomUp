...
import dev.tizwarp.ftbu.api.inventory.ContainerResearch;
import dev.tizwarp.ftbu.api.inventory.InventoryCraftingPersistent;
import dev.tizwarp.ftbu.api.inventory.SlotCrafting;
import dev.tizwarp.ftbu.api.technology.puzzle.gui.PuzzleGuiConnect;
...
public class PuzzleConnect implements IPuzzle {
	...
	private static boolean connects(ItemPredicate predicate, ItemStack stack) {
			...
			if (predicate.test(recipe.getRecipeOutput())) {
				for (Ingredient ingredient : recipe.getIngredients())
					if (ingredient.test(stack))
						return true;
			} else if (FTBUAPI.stackUtils.isEqual(recipe.getRecipeOutput(), stack))
				for (Ingredient ingredient : recipe.getIngredients())
		...
		for (Map.Entry<ItemStack, ItemStack> entry : FurnaceRecipes.instance().getSmeltingList().entrySet()) {
			if (predicate.test(entry.getValue()) && FTBUAPI.stackUtils.isStackOf(entry.getKey(), stack))
				return true;
			if (FTBUAPI.stackUtils.isEqual(entry.getValue(), stack))
				for (ItemStack s : predicate.getMatchingStacks())
					if (FTBUAPI.stackUtils.isStackOf(entry.getKey(), s))
						return true;
		}
		return false;
	}
	...
	private boolean fits(ItemStack stack, int index) {
		if (research.left.test(stack) || research.right.test(stack))
			return false;
		for (int i = 0; i < 3; i++)
			if (FTBUAPI.stackUtils.isEqual(stack, inventory.getStackInSlot(i)))
				return false;
